...
public class Info extends JFrame implements ActionListener, MouseListener, MouseMotionListener {
	...
	private JTextField txtrTest, txtrTest_1,  txtrTest_2;
	...
	public Info() {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("select * from myinfo");
			while(rs.next()) {
				Myid =rs.getString("id");
			}
			ResultSet rs2 = stmt.executeQuery("select * from logindata where id ='"+Myid+"'");
		
			
				while(rs2.next()) {
					txtrTest.setText(rs2.getString("username"));
					txtrTest_1.setText(rs2.getString("id"));
					txtrTest_2.setText(rs2.getString("callnum"));		
				}
	...
	public JTextField getTxtrTest_1() {
		return txtrTest_1;
	}

	public void setTxtrTest_1(JTextField txtrTest_1) {
		this.txtrTest_1 = txtrTest_1;
	}
